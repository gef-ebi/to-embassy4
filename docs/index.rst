...
On **Monday Nov. 1st 2021** a second and final part -- `extcloud05 <https://extcloud05.ebi.ac.uk/>`_ --
will be decommissioned. We therefore we need to have all remaining activities which use `extcloud05`
migrated to the new Embassy4 by **Friday Oct. 29th 2021**.
...
.. warning:: Our primary concern at this stage is that both |ECP| and BioExcel  portal application
   developers will need to make changes to their applications/VM\ s by October 29th so that they
   can be deployed to `Embassy4`. If it is not possible to do so before that date then the
 ...
 * The version of Terraform which we use for deployment purposes has changed and as a result the
   :file:`deploy.sh`, :file:`state.sh` and :file:`destroy.sh` scripts used to control the |VM| 
   lifecycle (e.g. https://github.com/EMBL-EBI-TSI/cpa-bioexcel-cwl/tree/master/ostack) need to change.
 ...
 * Week 1 (Sep. 27 - Oct 1)
    * Launch this website
    * Reach out to ECP/BioExcel portal users
    * Continue |VM| change requirement investigations
 * Week 2 (Oct 4 - Oct 8)
    * Finalise |VM| change requirement investigations
    * Update this website as new details arrive
    * Switch our |ECP| "development" environment [#f4]_ to deploy to `Embassy4`
 * Week 3 (Oct 11 - Oct 15)
    * Summary update via email - invite people to use our |ECP| "development" environment
    * Monitor |ECP| "development" environments
    * Continue to update this website as new details arrive
 * Week 4 (Oct 18 - Oct 22)
    * Switch over |ECP| and BioExcel "production" environments to using `Embassy4`
